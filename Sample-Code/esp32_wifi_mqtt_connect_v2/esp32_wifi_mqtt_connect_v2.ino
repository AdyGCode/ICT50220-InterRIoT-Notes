...
Adafruit_MQTT_Publish   pub_data = Adafruit_MQTT_Publish(&mqtt, "iot/data");
Adafruit_MQTT_Publish   pub_announce = Adafruit_MQTT_Publish(&mqtt, "iot/announce");
Adafruit_MQTT_Subscribe sub_command = Adafruit_MQTT_Subscribe(&mqtt, "iot/command");
...
void loop()
{
  ...
  pub_data_error = pub_data.publish((uint32_t)counter);

  if (!pub_data_error){
    DEBUG_PRINTLN("Pub: Failed");
  } else {
    DEBUG_PRINTLN("Pub: Success");
  }
