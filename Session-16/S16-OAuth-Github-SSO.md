...
https://itnext.io/fastapi-github-single-sign-on-sso-f6b942395649



## FastAPI: GitHub Single Sign-On (SSO)

![Image by author](../assets/1_8cvme2jBus1sTYShuax_-Q.webp)
...
In this article, we will demonstrate a Single Sign-On implementation in FastAPI to login users in a system via **GitHub sign-in**.
...
In our demonstration we will use FastAPI with basic credentials and cookie authentication. Along with the basic credentials traditional login form, a user will have also the option to sign in with GitHub and other methods.
...
In order to use the GitHub Single Sign-On mechanism, an app is needed to be created using the GitHub platform, let’s follow the steps together:

* Sign in your GitHub account and navigate to settings, then locate the “Developer settings”. Should be located on the bottom of settings section.

![](../assets/1_eTYdZ5Izw9gpLCi5Af8ItA.webp)

* Select the tab “GitHub Apps” from the left and press “New GitHub App”. If you have setup 2FA mechanism, an approval step is shown here.

* Give your GitHub app a name, a home page URL (just use the repository URL if there is no page yet) and define a “Callback URL”. This callback URL is the callback endpoint where the GitHub sign-in mechanism will redirect at, after its own authentication is done, passing along the user’s information. Notice that I’m using a localhost URI here. That’s because, I’ll try all that locally, since I’m not going to deploy that implementation on some host. Once and if your code is deployed on some server, then this URL should be updated to point to the exact IP/Domain and URL path for your callback endpoint.

![](../assets/1_afqnytQzcv6gzQlTd_H5xA.webp)

* Leave everything else as in default state and create the app. Note the “Client ID” that is provided. Press “Generate a new client secret” and note that value as well. We will use those credentials in a bit in our code configuration.

![](../assets/1_1ogmznX7NxyczC-KZ8v-bA.webp)
...
All we need to know about that user interface, is what happens when we click the GitHub sign-in button which is described by this piece of HTML code:
...
So, upon that button press (submit), the action that will be performed is to issue a GET request at relative path /v1/github/login*. *Let’s keep that in mind and see what that means for our API endpoints in a bit.
...
* The environment variables loaded are the GitHub Client ID and secret, the ones we received after setting up our GitHub app in Github platform.

* The GithubSSO class is included in the imported SSO library that we talked a bit earlier. Notice that during instatiation we need to provide our GitHub app credentials there, as well as our redirect URL, same as callback URL declared in our GitHub App. Additionally, since we need to test this locally and without any https protocol enabled, we need to declare that and allow insecure http traffic by setting the related argument to True.

* The /v1/github/loginendpoint is where the whole mechanism is triggered by. Remember that was the route that our GitHub sign-in button was targetting upon in our form. The function executed calls the get_login_redirect method of the github_ssoinstance which triggers the GitHub sign-in flow in our browser. Notice that the form window declares our GitHub app name, the one created in GitHub platform.

## Signing-in in via the User Interface

Let’s trigger the log in process using the GitHub button in our User Interface. This will redirect us here:

![](../assets/1_wa__NEgQBFGQ4-xvnz49Qg.webp)
...
![](../assets/1_rQweB9bMq4U9xyeHjHlZ4Q.webp)

* Once the GitHub sign-in process is finished, the GitHub app will redirect to our declared redirect URI, hence [http://localhost:9999/v1/github/callback](http://localhost:9999/v1/google/callback) which takes us back into our local application and the second endpoint in our code.
...
The username displayed is the GitHub username received from the GitHub SSO mechanism and the table presented are just stats from the local database. Notice that GitHub provider has authenticated one user so far.
>  That’s all folks!
