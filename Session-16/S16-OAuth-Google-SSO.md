...
https://itnext.io/fastapi-google-single-sign-on-sso-47454e2e2859

## FastAPI: Google Single Sign-On (SSO)

![Image by author](../assets/1_aG0-3ZBs1OzbAYy2Uk-Pog.webp)
...
In this article, we will demonstrate a Single Sign-On implementation in FastAPI to login users in a system via **Google sign-in**.
...
In our demonstration we will use FastAPI with basic credentials and cookie authentication. Along with the basic credentials traditional login form, a user will have also the option to sign in with Google and other methods.
...
In order to use the Google Single Sign-On mechanism, an app is needed to be created using the Google cloud console, let’s follow the steps together:

* Firstly, one will need a Google account and a project in Google Cloud Platform. So in order to perform the next steps you need to setup your account and access Google Cloud console to create a project. Do not worry about billing. Google cloud platform provides some credit points for each new account but we don’t need them anyway, as nothing in this demo is billable.

* While in Google Cloud console, search in the search bar for Credentials.

* Enable any APIs that is listed there as not enabled.

* Move to Credentials tab from the menu on the left

![Image by author](../assets/1_eRSxIsJpGOfQFFbMCtMuJQ.webp)

* Press “Create credentials” from the top bar menu and select the option “OAuth client ID” option

![Image by author](../assets/1_KnkEchECZT2dBhrfUC_5QA.webp)

* In the form, select the option web application, give any name you want and add a redirect URI. This URI is the callback endpoint where the Google sign-in mechanism will redirect at, after its own authentication is done, passing along the user’s information. Notice that I’m using a localhost URI here. That’s because, I’ll try all that locally, since I’m not going to deploy that implementation on some host. Once and if your code is deployed on some server, then this URI should be updated to point to the exact IP/Domain and URI path for your callback endpoint.

![Image by author](../assets/1_SBBGHIGdwgfpJNmExrUhAg.webp)

* Press “Create”, then this window will pop up:

![Image by author](../assets/1_LPhCwyhWydwSpI2rLCjznA.webp)

Make sure to download this JSON file, or copy the Client ID and secret values. Will use those later in our code configuration.
...
So, upon that button press (submit), the action that will be performed is to issue a GETrequest at relative path /v1/google/login*. *Let’s keep that in mind and see what that means for our API endpoints in a bit.
...
* The environment variables loaded are the Google Client ID and secret, the ones we received after setting up our Google app and credentials in Google Cloud Platform’s console.

* The GoogleSSO class is included in the imported SSO library that we talked a bit earlier. Notice that during instatiation we need to provide our app credentials there, as well as our redirect URI, same as the redirect URI declared in our Google app. Additionally, since we need to test this locally and without any https protocol enabled, we need to declare that and allow insecure http traffic by setting the related argument to True.

* The /v1/google/loginendpoint is where the whole mechanism is triggered by. Remember that was the route that our Google sign-in button was targetting upon in our user interface form. The function executed calls the get_login_redirect method of the google_sso instance which triggers the Google sign-in flow in our browser. Notice that the form window declares our Google app name, the one created in Google Cloud platform.

## Signing-in in via the User Interface

Let’s trigger the log in process using the Google button in our User Interface. This will redirect us here:

![Image by author](../assets/1_GloR_u_W2ostfHbCWl4Dtg.webp)
...
![Image by author](../assets/1_0y6M5_GpyVh1OD-XLJ9Puw.webp)
